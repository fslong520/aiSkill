...
- AVOID 无脑遍历就能解决的问题
- AVOID 题面太直白或太中二
- ⛔ **AVOID 模板题**——禁止以下"题库烂大街"的题型：
  - LCS（最长公共子序列）、LIS（最长上升子序列）、LPS（最长回文子序列）
  - 0-1背包、完全背包、多重背包裸题
  - 八皇后、全排列、N皇后（DFS模板）
  - 括号匹配、表达式求值（栈模板）
  - 活动选择、区间选点、排队打水（贪心模板）
  - 数字三角形、矩阵连乘（区间DP模板）
  - 走迷宫（BFS模板）
  - 最小生成树（Kruskal/Prim裸题）
  - 最短路径（Dijkstra/Floyd裸题）
  - 这些题本身不差，但OJ题库里已有上百份，再做就是浪费参赛者时间
  - **必须原创配题、非要让参赛者拐个弯才能发现算法本质**
...
- ✗ 机械的多步操作（转进制→统计1→相加）
- ✗ 无脑遍历就能解决
- ⛔ **禁止裸模板题**：任何能让选手一眼认出"哦这是XX算法"直接套模板的题都是不合格的。
  - 如果题面去掉故事后，选手能在5秒内说出算法名→不合格
  - 如果题面+算法名直接就是某经典题的题干→不合格
  - 必须加"拐弯"：要么加一个额外约束条件迫使选手无法直接套模板，
    要么把场景包装得让选手需要先建模转化才能触及核心算法，
    要么把两个看似不相关的知识点融合在一起。
...
- [ ] 样例是否具有代表性？
- [ ] 能否区分不同水平的选手？
- [ ] **⛔ 模板检查**：去掉故事包装后，赤裸的算法本质是不是某个OJ上出现超过3次的经典题？如果是→不合格
- [ ] **⛔ 拐弯检查**：选手能否在30秒内识别出算法并开始写模板代码？如果能→包装不够深，加拐弯
- [ ] **⛔ 自问"新在哪"**：这道题和已知题的区别是什么？答不出→不合格
