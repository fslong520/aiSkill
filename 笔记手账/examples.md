...
Style: notebook scrapbook, elegant, ink-blue, neat and graceful. Aspect ratio 3:4.
```
...
Section 2: 墨蓝 header "二、中序遍历（左-根-右）". White rounded card below with light shadow, 雾蓝 top color bar. "先递归左子树，再访问根节点，最后递归右子树" with example: D→B→E→A→F→C.

Section 3: 墨蓝 header "三、后序遍历（左-右-根）". White rounded card below with light shadow, 黛蓝 top color bar. "先递归左子树，再递归右子树，最后访问根节点" with example: D→E→B→F→C→A.
...
2. "计算机系统" diagram with CPU, memory, storage icons
3. Open book illustration with lightbulb

Small educational icons at bottom: notebook, pencil, lightbulb. Clean, orderly, age-appropriate for 12-year-old students.

Style: notebook scrapbook, educational, elegant, ink-blue, neat and graceful. Aspect ratio 3:4.
```
...
Section 2: 墨蓝 header "二、stack 栈". White rounded card below with light shadow, 雾蓝 top color bar. Stack icon in 黛蓝. "后进先出LIFO，只能操作栈顶" with常用操作: push, pop, top.

Section 3: 墨蓝 header "三、queue 队列". White rounded card below with light shadow, 黛蓝 top color bar. Queue icon in 黛蓝. "先进先出FIFO，队首队尾操作" with常用操作: push, pop, front.
