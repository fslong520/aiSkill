# Step 7: 生测试数据

## 目标

**生 25 组测试数据 → 只编 `{WORK_DIR}/mkin.h` 之 `test()` 函数**

## ⚠️ 唯一要记

```
生测试点 = 编 {WORK_DIR}/mkin.h 中 test() 函数
勿碰 {WORK_DIR}/mkdata.cpp（乃框架，毋须改）
```

## 测试数据分布

生 25 组测试数据，分布方案详见 **`references/testdata-design.md` → 一、数据分布框架**。

## 边界清单与 Hack 设计

按题目类型选取对应边界。详见 **`references/testdata-design.md`**：

| 章节 | 内容 |
|------|------|
| 二、题型感知边界清单 | 按数组/图论/树/字符串/DP/数学分类的边界列表 |
| 三、Hack 数据设计 | 各类常见错误的 Hack 数据模式 |
| 四、各规模数据生成建议 | case 3-20 的规模与构造建议 |

**设计步骤：**
1. 辨题目类型（数组/图论/树/字符串/DP/数学/构造）
2. 自 `references/testdata-design.md` 对应章节挑拣边界
3. 将边界写入 `mkin.h` 的 `test()` 函数

## test() 模板

```cpp
void test(int case_num, ofstream& fout) {
    // ============================================================
    // Subtask 0: 样例数据（直接复制题目样例）— case 1-2
    // ============================================================
    if (case_num == 1) {
        // 样例1 - 从题面逐字复制
    }
    else if (case_num == 2) {
        // 样例2 - 从题面逐字复制
    }

    // ============================================================
    // Subtask 1: 小规模随机数据 — case 3-5
    // ============================================================
    else if (case_num >= 3 && case_num <= 5) {
        // 建议：N 在 [1, 10] 范围内
    }

    // ============================================================
    // Subtask 1(续): 特殊性质数据 — case 6-8
    // ============================================================
    else if (case_num == 6) {
        // 据 references/testdata-design.md 选性质1
    }
    else if (case_num == 7) {
        // 据 references/testdata-design.md 选性质2
    }
    else if (case_num == 8) {
        // 据 references/testdata-design.md 选性质3
    }

    // ============================================================
    // Subtask 2: Hack 数据（针对常见错误写法）— case 9-11
    // ============================================================
    else if (case_num == 9) {
        // 见 references/testdata-design.md → 三、Hack 数据设计
    }
    else if (case_num == 10) {
        // 见 references/testdata-design.md → 三、Hack 数据设计
    }
    else if (case_num == 11) {
        // 见 references/testdata-design.md → 三、Hack 数据设计
    }

    // ============================================================
    // Subtask 3: 中等规模数据 — case 12-15
    // ============================================================
    else if (case_num >= 12 && case_num <= 15) {
        // 建议：N 在 [100, 10000] 范围内
    }

    // ============================================================
    // Subtask 3(续): 大规模数据（压力测试）— case 16-20
    // ============================================================
    else if (case_num >= 16 && case_num <= 20) {
        // 建议：N 接近题目限制上限
    }

    // ============================================================
    // Subtask 4: 随机回归测试 — case 21-25
    // ============================================================
    else {
        // 随机生成，覆盖各种情况
    }
}
```

## 改 test() 后必同更

1. `mkin.h` 顶 `SUBTASKS[]` 数组（仅作注标记用）
2. `{WORK_DIR}/testdata/config.yaml` 格式见 **`references/testdata-design.md` → 五、config.yaml 格式**
3. 25 组测点总分持 100

## 配置文件（HydroOJ subtask 格式）

```bash
cd {WORK_DIR}
g++ -o mkdata mkdata.cpp -std=c++17
./mkdata
```

## 大样例处理

| 大小 | 处理 |
|------|------|
| < 500 字节 | `read_file` 读取 |
| ≥ 500 字节 | 禁止 `read_file`，用 shell 验证 |
## 验证

生数后必验：
1. 编译 std.cpp 生标程
2. 用前几组数据验标程输出正
3. 查 .in 与 .out 文件成对存否

## 追加测试点分布表至题面

生数据后，须于 `{WORK_DIR}/problem_zh.md` 末追加 **测试点分布表**，格式如下：

```markdown

---

### 测试点分布

| Subtask | 分值 | 测试点编号 | 说明 |
|:-------:|:----:|:----------:|:-----|
| 0 | 10 | 1~2 | 样例 |
| 1 | 20 | 3~8 | 小规模 N≤10 / 特殊: 全同值 / 特殊: 单调 |
| 2 | 15 | 9~11 | Hack: N=1边界 / Hack: 大数 / Hack: 超时 |
| 3 | 30 | 12~20 | 中规模 N≈1000 / 大规模 N≈1e5 压力 |
| 4 | 25 | 21~25 | 随机 N=1~1e5 回归 |
```

各列说明：
- **Subtask**：对应 config.yaml 中 subtask.id
- **分值**：该 subtask 的 score
- **测试点编号**：该 subtask 覆盖的 case 编号范围
- **说明**：描述该组输入的大致规模与数据特征（如 N 范围、特殊构造类型），让选手了解测点覆盖了什么场景

⚠️ **约束**：
- ✅ 可写输入规模范围（N≤10、N≈1e5 等）
- ✅ 可写构造类型（全同值、单调、链、菊花图、质数等）
- ❌ 禁写答案（ans=xx、输出 Yes/No 等）
- ❌ 禁写具体计算值（d=99991、S=1e9 等）
- ❌ 禁逐 case 列表（只按 subtask 分组，不拆到每个 case）

追加方式：

```bash
# 在 problem_zh.md 的 </div> 前插入测试点表
# 用编辑工具在 `</div>` 前添加上述 markdown 表格
```

## 下一步

成 → `08-audit.md`
