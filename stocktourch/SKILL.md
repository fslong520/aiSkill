...
- AI 需要主动协调：数据获取 → 财报下载 → 财报解析 → 网络搜索 → 综合分析 → 生成报+
...
**对于综合分析（个股）**：
AI 必须主动执行以下完整流程：
1. 使用 Python 脚本获取技术数据（基本信息、技术指标、基本面数据、行业分类等）
2. **必须调用** `core/financial_analyzer.py` 的 `get_comprehensive_financial_report()` 方法获取 AKShare 财务数据
3. **必须按照** `report/读财报.md` 方法论深度分析财报
4. **必须调用** `search_web` 工具搜索最新网络信息
