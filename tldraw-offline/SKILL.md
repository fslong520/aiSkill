...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/home/fslong/.config/tldraw/server.json`.
...
```bash
PORT=$(jq -r .port '/home/fslong/.config/tldraw/server.json'); TOKEN=$(jq -r .token '/home/fslong/.config/tldraw/server.json')
