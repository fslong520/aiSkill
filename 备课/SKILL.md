...
metadata:
  ...
  version: 2.3 (受众更新：6年级竞赛生，认知约当16岁)
---
...
| **图文并茂** | 每个抽象概念至少配一张示意图 | 学生看文字能懂吗？还是画出来更好？ |
| **逻辑连贯** | 知识点之间有自然的因果链条 | 为什么先讲这个再讲那个？换顺序可以吗？ |
| **因材适教** | 语言、类比、节奏匹配竞赛生认知（虽年6年级，然知约16岁） | 这课对于这个认知层级的学生是有趣还是无聊？ |
...
- AVOID 纯文字讲义/PPT 无配图——6年级竞赛生需要视觉辅助理解抽象概念
- AVOID 插图风格不统一——同一系列课件应锁定一种画风（卡通/扁平/信息图）
- AVOID 配图与内容脱节——每张图必须有明确教学目的，不为凑图而加图
- AVOID 流程图/结构图纯手打 ASCII——能用图片姬+Agnes画影生成专业配图，不用简陋字符画
- AVOID 全程无图干讲——每节至少一张核心概念示意图（树形图/流程图/对比图/类比图）
- AVOID Marp 单页超 25 行不加拆分——超过即拆，一图一要点，不堆砌
