...
本技能默认使用 **bge-base-zh-v1.5**（BAAI 中文语义模型，768 维），回退 all-MiniLM-L6-v2（384 维）。
引擎自动检测 `~/.local/share/opencode/忆时/models/`，不存在则自动下载或回退。
安装命令、维度变更重建、离线包回退——详见 modules/08-setup.md。
